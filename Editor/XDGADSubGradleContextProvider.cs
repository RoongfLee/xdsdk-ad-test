...
namespace XD.SDK.ADSubPackage
{
    public class XDGADSubGradleProvider : IPreprocessBuildWithReport
    {
        ...
        private static void SetNativeAarCompatible(string aarFileName, bool androidCompatibleEnable)
        {
            ...
            try
            {
                var aarFilePath = GetNativeAarFilePath(aarFileName);
        ...
        /// 获取本地 aar 文件路径
        /// </summary>
        /// <param name="fileName">文件名（不能包含 .aar 后缀）</param>
        /// <returns>返回 {fileName} 对应的文件路径</returns>
        private static string GetNativeAarFilePath(string fileName)
        {
            var resultPath = "";
            ...
            foreach (var guid in fileGuids)
            {
                ...
                var fileInfo = new FileInfo(findPath);
                if (!fileInfo.Name.EndsWith($"{fileName}.aar")) continue;
                resultPath = findPath;
                break;
            }
