...
namespace XD.SDK.ADSubPackage
{
    ...
    {
        private readonly string _humeSDKPath = "Assets/XDSDK/Mobile/ADSubPackage/Plugins/Android/libs/humesdk-1.0.0.aar";
        private readonly string _gdtActionSDKPath = "Assets/XDSDK/Mobile/ADSubPackage/Plugins/Android/libs/GDTActionSDK.min.1.8.9.aar";
        ...
        private List<AndroidGradleContext> GetGradleContextByXDConfig()
        {
            ...
            if (configObject["ad_config"]?["tt_config"] != null)
            {
                ...
                // 今日头条分包 SDK
                SetNativeAarCompatible(_humeSDKPath, true);
            }
            else
            {
                // 今日头条分包 SDK
                SetNativeAarCompatible(_humeSDKPath, false);
            }
            ...
            if (configObject["ad_config"]?["gdt_config"] != null)
            {
                ...
                // 广点通广告 SDK
                SetNativeAarCompatible(_gdtActionSDKPath, true);
            }
            else
            {
                // 广点通广告 SDK
                SetNativeAarCompatible(_gdtActionSDKPath, false);
            }
        ...
        /// 修改本地 aar 文件属性
        /// </summary>
        /// <param name="nativeAarPath">本地 aar 文件路径</param>
        /// <param name="androidCompatibleEnable">针对 Android 平台的兼容性是否可用</param>
        private static void SetNativeAarCompatible(string nativeAarPath, bool androidCompatibleEnable)
        {
            Debug.Log("SetNativeAarCompatible -----> " + nativeAarPath + ", enable: " + androidCompatibleEnable);
            try
            {
                var pluginImporter = AssetImporter.GetAtPath(nativeAarPath) as PluginImporter;
                if (pluginImporter == null)
                {
                    Debug.LogWarning("can't get native aar library AssetImporter: " + nativeAarPath);
                    return;
                }
            ...
            Debug.LogWarning("can't disable native aar library: " + nativeAarPath);
        }
    }
}
